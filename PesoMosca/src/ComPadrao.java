...
public class ComPadrao {
    public static void main(String[] args) {
        ...
        for(int i=0; i< 200000; i++){
            FlyweightStatusItem fItem = new FlyweightStatusItem();
            Pedido pedido = new Pedido();
            Item item1 = new Item();
            item1.setDescricao("lapis de cor modelo xyz");
            item1.setNome("lapis");                        
            item1.setStatus(fItem.get(Estado.CARRINHO));
            Item item2 = new Item();
            item2.setDescricao("caneta cor azul");
            item2.setNome("caneta");                                    
            item2.setStatus(fItem.get(Estado.CARRINHO));
            Item item3 = new Item();
            item3.setDescricao("caneta cor vermelha");
            item3.setNome("caneta");                        
            item3.setStatus(fItem.get(Estado.CARRINHO));                       
